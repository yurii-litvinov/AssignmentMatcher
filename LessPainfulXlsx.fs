...
let openXlsxSheet (name: string) sheetNumber =
    let dataDocument = SpreadsheetDocument.Open(name, false)
    let workbookPart = dataDocument.WorkbookPart
    let worksheetPart = workbookPart.WorksheetParts |> Seq.skip sheetNumber |> Seq.head
